...
from app.services.ai_scorer import extract_stats_from_pdf
...
async def upload_stats(match_id: str, file: UploadFile = File(...)):
    ...
    stats_json = await extract_stats_from_pdf(content, match_id)
    return {"status": "pending_approval", "extracted_stats": stats_json}
...
def _update_impact_rating(player_id: str):
    ...
    matches = len(rows)
    impact = round((total_runs / max(matches, 1)) * 0.6 + (total_wickets / max(matches, 1)) * 15 * 0.4, 2)
